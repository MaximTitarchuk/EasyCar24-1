    ...
    MINOVATE.extra = {
        ...
        splash: function () {
            ...
            $splashEl.on('show.bs.modal', function (e) {
                options = typeof e.relatedTarget == "undefined" ? e.target.dataset.options : e.relatedTarget.dataset.options;
                target = $(e.target);
...
$(document).ready(function () {
    $(".save-button").click(function () {
        ...
        var data = {};
        // $.each($("#edit input,textarea"), function (idx, obj) {
        //     if ($(obj).is("input[type='text']") || $(obj).is("input[type='hidden']") || $(obj).is("input[type='tel']"))
        //         data[$(obj).attr("id")] = $(obj).val();
        //     else if ($(obj).is("textarea"))
        //         data[$(obj).attr("id")] = tinyMCE.get($(obj).attr("id")).getContent();
        //     else if ($(obj).is("input[type='checkbox']"))
        //         data[$(obj).attr("id")] = $(obj).is(":checked")? 1: 0;
        //     else
        //         toastr.error("Не определено событие для элемента #" + $(obj).attr("id"), 'Ошибка', toastrOptions);
        //
        //     if (data[$(obj).attr("id")] == "")
        //         data[$(obj).attr("id")] = null;
        // });

        $("#edit form").serializeArray().map(function(item) {
            if ( data[item.name] ) {
        ...
        if ($("#edit form").find("textarea.mce").length > 0)
            $.each($("#edit form").find("textarea.mce"), function(idx, item) {
                data[item.name] = tinyMCE.get(item.id).getContent();
            });

        data["_token"] = $('meta[name="csrf-token"]').attr('content');

        $.ajax({
            url: $("#edit-data").data("save-action"),
            data: data,
            method: id == ""? 'POST': 'PUT'
        }).done(function() {
    ...
    $(".add-button").click(function () {
        $("#edit h3").html("Добавить данные");
        $.each($("#edit input, #edit textarea"), function (idx, obj) {
            if ($(obj).is("input[type='text']") || $(obj).is("input[type='hidden']") || $(obj).is("input[type='email']") || $(obj).is("input[type='tel']"))
                $(obj).val("");
            else if ($(obj).is("textarea"))
                tinyMCE.get($(obj).attr("id")).setContent("");
            else if ($(obj).is("input[type='checkbox']"))
                $(obj).prop("checked", true);
            else if ($(obj).is("input[type='radio']"))
                $(obj).prop("checked", false);
        });
        $(".onoffswitch-view input[type='checkbox']").trigger("change");
        $("#edit").modal("show");
    });

    $("#list").on("click", ".remove-button", function () {
        $(".remove-сonfirm-button").attr("data-id", $(this).data("id"));
        $("#remove").modal("show");
    });

    $(".remove-сonfirm-button").click(function () {
        $.ajax({
            url: $("#edit-data").data("delete-action"),
            data: {id: $(this).attr("data-id"), _token: $('meta[name="csrf-token"]').attr('content')},
            method: "delete"
        }).done(function() {
            toastr.success('Данные успешно удалены', 'Удаление', toastrOptions);
            oTable.ajax.reload( null, false );
            $("#remove").modal("hide");
        })
        .fail(function(data) {
    ...
    $("#list").on("click", ".restoreBtn", function () {
        $.ajax({
            url: $("#edit-data").data("restore-action"),
            data: {id: $(this).data("id"), _token: $('meta[name="csrf-token"]').attr('content')},
            method: "put"
        }).done(function() {
            toastr.success('Данные успешно восстановлены', 'Восстановление', toastrOptions);
            oTable.ajax.reload( null, false );
        })
        .fail(function(data) {
            var error = "";
    ...
    $("#list").on("click", ".edit-button", function () {
        ...
        $("#edit h3").html("Изменение данных");
        $("#edit .save").attr("data-id", id);
        $(".onoffswitch-view input[type='checkbox']").trigger("change");

        $.getJSON( $("#edit-data").data("info-action"), {id: id})
            .done(function( data ) {
                        ...
                        if (typeof value == 'object' && Object.keys(value).length > 0) {
                            $.each(value, function (idx, value) {
                                if ($("#" + field).data("type") == "text")
                                    addTextElement($("#" + field), "text", field + '_' + value.id, field + '[' + value.id + ']', value.value);
                                else if ($("#" + field).data("type") == "phone")
                                    addTextElement($("#" + field), "tel", field + '_' + value.id, field + '[' + value.id + ']', value.value, 'data-inputmask="\'mask\' : \'9 (999) 999-9999\'"');
                            });
                            $("input[type='tel']").inputmask();
                        }
                        else if ($("#" + field).is("input[type='text']") || $("#" + field).is("input[type='email']") || $("#" + field).is("input[type='url']") || $("#" + field).is("input[type='tel']"))
                            $("#" + field).val(value);
                        else if ($("#" + field).is("select")) {
                            console.log("#" + field + " option[value='" + value + "']");
                            $("#" + field + " option[value='" + value + "']").prop("selected", true);
                        }
                        else if ($("#" + field).is("textarea"))
                            tinyMCE.get(field).setContent(value);
                        else if ($("#" + field).is("input[type='checkbox']")) {
                            $("#" + field).prop("checked", value == 1 ? true : false).trigger("change");
                        }
                        else if ($("[name='" + field + "']").is("input[type='radio']")) {
                            $("[name='" + field + "'][value='" + value + "']").prop("checked", true).trigger("click");
                        }
                        else
                            toastr.error("Не определено событие для элемента #"+field, 'Ошибка', toastrOptions);
                    }
                });
                $("#id").val(id);
                $("#edit").modal("show");
    ...
    $("#list").on("click", ".viewBtn", function () {
        var id = $(this).data("id");
        var view = 1 - Number($(this).data("view"));

        $.ajax({
            url: "/pages/view",
            data: {id: id, view: view, _token: $('meta[name="csrf-token"]').attr('content')},
            method: 'PUT'
        }).done(function() {
            toastr.success('Данные успешно сохранены', 'Сохранение', toastrOptions);
            oTable.ajax.reload( null, false );
    ...
    $(".onoffswitch-view input[type='checkbox']").change(function () {
        if ($(this).is(":checked"))
            $(this).parent().find(".onoffswitch-view-switch").css("right", "1px");
        else
            $(this).parent().find(".onoffswitch-view-switch").css("right", "74px");
    });

    $("#edit").on("click", ".element-delete", function() {
...
//          content_css : '/css/app.css',
    ...
    external_plugins: {"filemanager": "/assets/js/tinymce/filemanager/plugin.min.js"},
    toolbar: "undo redo | bold italic underline | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | styleselect forecolor backcolor | link unlink | responsivefilemanager imagemedia | code",
    setup: function (ed) {
