...
{
    private $charset;
    private $HTTPS_ADDRESS 	= "https://lcab.smsintel.ru/";
    private $HTTP_ADDRESS 	= "http://lcab.smsintel.ru/";
    private $HTTPS_METHOD	= "curl";
    private $USE_HTTPS		= 1;
    private $HTTPS_CHARSET_AUTO_DETECT = false;
    private $HTTPS_CHARSET	= "utf-8";
    ...
    function send($params = array(), $phones = array()){
        ...
        $someXML = "";
        if (isset($params["text"])) $params["text"] = htmlspecialchars($params["text"],null,$this->HTTPS_CHARSET);
        foreach ($phones as $phone){
            if (is_array($phone)){
                if (isset($phone["number"])){
                    $someXML .= "<to number='".$phone['number']."'>";
                    if (isset($phone["text"])){
                        $someXML .= htmlspecialchars($phone["text"],null,$this->HTTPS_CHARSET);
                    }
                    $someXML .= "</to>";
                }
            }
    ...
    function getURL($action){
        if ($this->USE_HTTPS == 1)
            $address = $this->HTTPS_ADDRESS."API/XML/".$action.".php";
        else
            $address = $this->HTTP_ADDRESS."API/XML/".$action.".php";
    ...
    function request($action,$params = array(),$someXML = ""){
        ...
        if ($this->HTTPS_METHOD == "curl"){
            $res = $this->request_curl($action,$xml);
        }
        elseif ($this->HTTPS_METHOD == "file_get_contents"){
            $opts = array('http' =>
                array(
                    'method'  => 'POST',
        ...
        $this->error("В настройках указан неизвестный метод запроса - '".$this->HTTPS_METHOD."'");
    }
    ...
    function makeXML($params,$someXML = ""){
        ...
        <data>
                        <login>".htmlspecialchars(config('sms.login'),null)."</login>
                        <password>".htmlspecialchars(config('sms.password'),null)."</password>
                        ";
    ...
    function getConvertedString($value, $from = false){
        if ($this->HTTPS_CHARSET_AUTO_DETECT){
            if (!$this->charset){
                $this->charset = $this->detectCharset($value);
            }
        }
        else{
            $this->charset = $this->HTTPS_CHARSET;
        }
